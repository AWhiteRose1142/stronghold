...
function Tower:initialize( height, position, layer, health )
  ...
  self.baseDeck = ResourceManager:get( 'towerBase' )
  self.topDeck = ResourceManager:get( 'towerTop' )
  self.midDeck = ResourceManager:get( 'towerMiddle' )
  ...
  table.insert( Level.playerEntities.tower, self )
end
...
function Tower:damage( damage )
  self.health = ( self.health - damage )
  
  if self.health <= 0 then
    print( "this tower is destroyed" )
  end
end
...
function Tower:getTransform()
  return self.transform
end

function Tower:getTopLoc()
...
function Tower:mountEntity( mount )
...
function Tower:initializePhysics( position, height )
  self.physics = {}
